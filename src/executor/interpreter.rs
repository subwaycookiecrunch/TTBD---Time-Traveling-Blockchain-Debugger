use crate::core::{HaltReason, VmError, VmResult, U256};
use crate::executor::Opcode;
use crate::journal::{Checkpoint, InstructionJournal, JournalEntry, StateSnapshot};
use crate::vm::Vm;
    ...
    pub fn step_forward(&mut self) -> VmResult<StepResult> {
        if self.state.pc >= self.bytecode.len() {
            return Ok(StepResult::Halted {
                reason: HaltReason::Stop,
            });
        }

        let opcode_byte = self.bytecode[self.state.pc];
        let opcode = Opcode::from_u8(opcode_byte).ok_or(VmError::InvalidOpcode {
            opcode: opcode_byte,
        })?;
        ...
        insn_journal.push(JournalEntry::GasChange {
            old_gas,
            new_gas: self.state.gas,
        });
    ...
    fn execute_opcode(
        ...
        match opcode {
            ...
            Opcode::Lt => {
                ...
                let b = self.state.stack.pop()?;
                journal.push(JournalEntry::StackPop { value: b });
                let result = if a.as_u64() < b.as_u64() {
                    U256::ONE
                } else {
                    U256::ZERO
                };
            ...
            Opcode::SStore => {
                ...
                journal.push(JournalEntry::StorageWrite {
                    key,
                    old_value,
                    new_value: value,
                });
            ...
            Opcode::Jump => {
                ...
                if !self.is_valid_jump(dest_usize) {
                    return Err(VmError::InvalidJump {
                        destination: dest_usize,
                    });
                }
                journal.push(JournalEntry::PcChange {
            ...
            match self.step_forward()? {
                StepResult::Halted { reason } => {
                    ...
                    return Ok(match reason {
                        HaltReason::Stop => ExecutionResult::Success {
                            return_data: Vec::new(),
                            gas_used,
                        },
                        HaltReason::Return(data) => ExecutionResult::Success {
                            return_data: data,
                            gas_used,
                        },
                        HaltReason::Revert(data) => ExecutionResult::Revert {
                            return_data: data,
                            gas_used,
                        },
